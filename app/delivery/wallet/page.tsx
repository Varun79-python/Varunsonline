...
export default function DeliveryWallet() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) return
      const { data: ag } = await supabase.from('delivery_agents').select('wallet_balance, today_earnings, total_earnings').eq('id', user.id).single()
  ...
  async function requestWithdraw() {
    ...
    if (Number(form.amount) < 50) { setMsg('Minimum withdrawal is ₹50'); return }
    setSubmitting(true)
    const { data: { user } } = await supabase.auth.getUser()
    await supabase.from('withdraw_requests').insert({ user_id: user?.id, user_type: 'delivery_agent', amount: Number(form.amount), payment_method: form.payment_method, upi_id: form.upi_id, bank_account_number: form.bank_account_number, bank_ifsc: form.bank_ifsc })
  ...
  return (
    <div className="fade-in" style={{ maxWidth: 560, margin: '0 auto' }}>
      {/* Balance Card */}
      <div style={{ background: 'linear-gradient(135deg, rgba(249,115,22,0.2), rgba(34,197,94,0.1))', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 28, textAlign: 'center', marginBottom: 24 }}>
        ...
        <button className="btn btn-primary" onClick={() => setShowWithdraw(true)} disabled={balance < 50}>Withdraw →</button>
      </div>
      ...
      {showWithdraw && (
        <div className="modal-overlay">
          <div className="modal">
            <div className="modal-header"><h3>💸 Withdraw Funds</h3><button className="modal-close" onClick={() => setShowWithdraw(false)}>✕</button></div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {form.payment_method === 'upi' && <div className="input-group"><label className="input-label">UPI ID</label><input className="input" value={form.upi_id} onChange={e => setForm(f => ({ ...f, upi_id: e.target.value }))} /></div>}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {txns.length === 0 && <div className="card" style={{ textAlign: 'center', padding: 30 }}>No transactions yet</div>}
        {txns.map(t => (
