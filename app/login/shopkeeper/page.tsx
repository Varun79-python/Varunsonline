...
export default function ShopkeeperLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (user) {
      ...
      // Check if documents not uploaded (shop_image_url is the key indicator)
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, padding: '0 24px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div style={{ textAlign: 'center', marginTop: 20 }}>
          ...
          <button onClick={() => router.push('/register?type=shopkeeper')} style={{ background: 'none', border: 'none', color: '#0ea5e9', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer' }}>Register</button>
        </div>
      </div>
