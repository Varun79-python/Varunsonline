...
-- ============================================================
DROP POLICY IF EXISTS "Shopkeeper views own subscriptions" ON public.shop_subscriptions;
DROP POLICY IF EXISTS "Shopkeeper inserts own subscriptions" ON public.shop_subscriptions;
DROP POLICY IF EXISTS "Admin manages subscriptions" ON public.shop_subscriptions;

CREATE POLICY "Shopkeeper views own subscriptions" ON public.shop_subscriptions
  FOR SELECT USING (shop_id IN (SELECT id FROM public.shops WHERE owner_id = auth.uid()));

CREATE POLICY "Shopkeeper inserts own subscriptions" ON public.shop_subscriptions
